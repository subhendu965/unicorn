...
<?php
	$query="SELECT * FROM routes WHERE name='".mysqli_real_escape_string($db,$_GET['name'])."' LIMIT 1";
	if($rows=mysqli_query($db,$query)){
		if($row=mysqli_fetch_array($rows,MYSQLI_BOTH)){
			if($row['name']==$_GET['name']){
				$_SESSION['admin']['notification_edit_id']=$row['name'];
			}else{
	...
	$stpids=Array();
	$stpNames=Array();
	$query="SELECT * FROM route_details WHERE name='".mysqli_real_escape_string($db,$_GET['name'])."'";
	if($rows2=mysqli_query($db,$query)){
		$i=0;
		while($row2=mysqli_fetch_array($rows2,MYSQLI_BOTH)){
			$stpids[$i]=$row2['stop_id'];
			$stpNames[$i]=idToStop($row2['stop_id']);
			$i++;
		}
		//print_r($stpids);
		$strStpid=implode("~#~",$stpids);
		$strStpName=implode("~#~",$stpNames);
	}else{
		echo "<h1 style='color:red'>Unable to get data.</h1>";
	}
...
<br>
<label><div>
Route: <input type="text" name="route" hb_code='input' value="<?php echo $row['name']; ?>" />
</div></label>

<label><div>
AC Route: <select name="ac" hb_code='input'><option value="1" <?php if($row['ac']==1){echo "selected";} ?>>Yes</option><option value="0" <?php if($row['ac']==0){echo "selected";} ?>>No</option></select>
</div></label>


<label><div>
Nonstop: <select name="nonstop" hb_code='input'><option value="1" <?php if($row['nonstop']==1){echo "selected";} ?>>Yes</option><option value="0" <?php if($row['nonstop']==0){echo "selected";} ?>>No</option></select>
</div></label>


<label><div>
Standing: <select name="standing" hb_code='input'><option value="1" <?php if($row['standing']==1){echo "selected";} ?>>Yes</option><option value="0" <?php if($row['standing']==0){echo "selected";} ?>>No</option></select>
</div></label>

<label><div>
Source: <input type="search" name="source" hb_code='input' onkeyup="searchKey(this,'depot')" onfocus="changeTarget('source')" autocomplete="off"  value="<?php echo $stpNames[0]; ?>" />
</div></label>

<label><div>
Destination: <input type="search" name="destination" hb_code='input' onkeyup="searchKey(this,'depot')" onfocus="changeTarget('destination')" autocomplete="off"  value="<?php echo $stpNames[count($stpNames)-1]; ?>" />
</div></label>
...
<?php
if(isset($strStpid) && !empty($strStpid) && isset($strStpName) && !empty($strStpName)){
?>
//alert()
addTarget='stop';
var stopToEdit54d_id = "<?php echo $strStpid; ?>".split("~#~");
var stopToEdit54d_name = "<?php echo $strStpName; ?>".split("~#~");

for(var i=0; i<stopToEdit54d_id.length; i++){
	addTargetToDiv(stopToEdit54d_id[i],stopToEdit54d_name[i]);
}
